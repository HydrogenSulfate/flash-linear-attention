...
class KimiDeltaAttention(nn.Module):
    def __init__(self, config: KimiLinearConfig, layer_idx: int):
        ...
        self.dt_bias = nn.Parameter(torch.empty(projection_size, dtype=torch.float32).fill_(-1.5))
    ...
    def __init__(self, config: KimiLinearConfig):
        ...
        if getattr(config, "_attn_implementation", None) is not None:
            if config._attn_implementation != "flash_attention_3":
                ...
                logger.warning_once("Using flash_attention_3 backend instead.")
...
class KimiLinearForCausalLM(KimiPreTrainedModel, FLAGenerationMixin):
    # _tied_weights_keys = ["lm_head.weight"]
    @property
    def _tied_weights(self):
        tied = {}
        if hasattr(self, "lm_head") and hasattr(self.lm_head, "weight"):
            tied["lm_head.weight"] = self.lm_head.weight
        return tied
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        fuse_linear_and_cross_entropy = (
            self.config.fuse_cross_entropy and self.training and labels is not None
        )
