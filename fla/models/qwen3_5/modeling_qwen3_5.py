...
class Qwen3_5DynamicCache:
    """
    A dynamic cache that can handle both the attention cache and KDA linear attention cache.
    """
    ...
    # 【新增】：让 KDA 可以通过 past_key_values[layer_idx] 读取状态
    def __getitem__(self, layer_idx: int):
        if layer_idx < len(self):
            # 初始状态为空时必须返回 None，防止 KDA 解包 Tuple 报错
            if self.conv_states[layer_idx] is None and self.recurrent_states[layer_idx] is None:
                return None
            return {
                "conv_state": self.conv_states[layer_idx],
                "recurrent_state": self.recurrent_states[layer_idx]
            }
        raise IndexError(f"Layer index {layer_idx} out of bounds.")

    # 【修改】：兼容全注意力机制的 update 与 KDA 的 kwargs update
    def update(
        self,
        ...
        **kwargs
        ...
        # 2. 如果是 KDA (Linear Attention) 调用
        if recurrent_state is not None or conv_state is not None:
            # logger.info(f"layer {layer_idx}: recurrent_state={'None' if recurrent_state is None else recurrent_state.shape}")
    ...
    # 【修改】：使 Beam Search 重排兼容 KDA 的 Tuple conv_states
    def reorder_cache(self, beam_idx: torch.LongTensor):
        for layer_idx in range(len(self.key_cache)):
            ...
            if self.conv_states[layer_idx] is not None:
                # KDA 的 conv_state 是 Tuple: (conv_q, conv_k, conv_v)
                if isinstance(self.conv_states[layer_idx], tuple):
                    device = self.conv_states[layer_idx][0].device
                    beam_idx = beam_idx.to(device)
                    self.conv_states[layer_idx] = tuple(
                        state.index_select(0, beam_idx) if state is not None else None
                        for state in self.conv_states[layer_idx]
                    )
                else:
...
class Qwen3_5DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3_5TextConfig, layer_idx: int):
        ...
        if self.layer_type == "linear_attention":
            if config.linear_type == "gdn":
                self.linear_attn = Qwen3_5GatedDeltaNet(config, layer_idx)
            elif config.linear_type == "kda":
                self.linear_attn = KimiDeltaAttention(
                    ...
                    num_v_heads=config.linear_num_key_heads,
                    use_short_conv=True,
                ...
                raise ValueError(f"Invalid linear_type: {config.linear_type}")
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.layer_type == "linear_attention":
            if self.linear_type == "gdn":
            ...
            elif self.linear_type == "kda":
                # hidden_states, _, past_key_values = self.linear_attn(
                #     hidden_states=hidden_states,
                #     attention_mask=attention_mask,
                ...
                #     past_key_values=past_key_values,
                #     cache_position=cache_position,
                #     # use_cache=use_cache,  # <<< 加入这一行，极其关键 <<<
                #     **kwargs,
                # )
                # actual_use_cache = kwargs.pop("use_cache", past_key_values is not None)
                # if past_key_values is not None:
                #     actual_use_cache = True
                kda_use_cache = past_key_values is not None
                _ = kwargs.pop("use_cache", None)  # 消耗掉避免kwargs冲突
                # logger.info(f"[KDA layer {self.layer_idx}] use_cache={actual_use_cache}, past_key_values={past_key_values is not None}")
                ...
                hidden_states, _, past_key_values = self.linear_attn(
                    ...
                    past_key_values=past_key_values,
                    cache_position=cache_position,
                    use_cache=kda_use_cache,  # <--- 传入强制判定后的状态
                    **kwargs,
                )
            else:
                raise RuntimeError
...
class Qwen3_5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # elif isinstance(module, KimiDeltaAttention) and next(module.parameters()).device.type != "meta":
        #     with torch.no_grad():
...
class Qwen3_5TextModel(PreTrainedModel):
    ...
    def _update_linear_attn_mask(self, attention_mask, cache_position):
        ...
        #     f"all_ones={attention_mask is not None and torch.all(attention_mask==1)}, "
        #     f"linear_attn_mask={'None' if linear_attn_mask is None else linear_attn_mask.shape}")
